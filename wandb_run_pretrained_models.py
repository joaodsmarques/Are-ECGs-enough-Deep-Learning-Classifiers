...
if __name__ == '__main__':
    ...
    model = ut.choose_model(config["model_type"], config["submodel"], 9, config)
    print(model)
    #Load model weights
    model.load_state_dict(checkpoint['model_state_dict'])
    

    #get number of features and change output layer
    ...
    for epoch in tqdm(range(config['epochs'])):
        ...
        if config['save_model']:
            ...
            if max_f1 < metrics['f1_score_overall']:
                ...
                pth_path = os.path.join(config['path_to_pth_file'], f"{config['dataset']}_{config['model_type']}_{config['submodel']}_pretrained.pth")
                torch.save({
